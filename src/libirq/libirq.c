...
static libirq_source_t* alloc_irq_source(gpio_num_t pin_irq,
                                         bool trigger_high) {
...
static void isr_irq(void* arg) {
    ...
    // First, since this interrupt is level-triggered (so we don't miss any)
    // disable further interrupts. If the waiting task happens to enable this
    // interrupt again asynchronously with this, we don't mind, since
    // `sleep_until_irq_active()` clears spurious pending messages.
    gpio_intr_disable(src->pin_irq);

    // Only now give the semaphore, which will ensure that the `irq_loop()` will
    // always eventually take the semaphore and call `gpio_intr_enable()` for
    // us. (We don't care about the return value, i.e. whether we were able to.)
    xEventGroupSetBits(src->events, IRQ_EVENT_GOT_IRQ);
}

// Returns `true` if the sleep ended because the IRQ became active,
// else returns `false` if the sleep was interrupted by a call for the
// underlying IRQ source to destroy itself.
static bool sleep_until_irq_active(const libirq_source_t* src) {
    // Clear a potential previous IRQ message from the IRQ ISR. Note that this
    // call is safe to make even if other tasks are currently waiting on this
    // bit.
    //
    // It is important that this bit is never cleared after a call to
    // `gpio_intr_enable`(), e.g. upon its reciept below, because this can
    // result in a race condition (as explained below).
    xEventGroupClearBits(src->events, IRQ_EVENT_GOT_IRQ);

    // First, enable the IRQ ISR. This is safe to call if another task has
    // already done this and is currently waiting (but in principle could cause
    // a "race" where both the we and the other task observe the "first" IRQ to
    // arrive, while the ISR triggers again due to this call and sets the
    // `IRQ_EVENT_GOT_IRQ` bit again "supriously"). This corner-case is dealt
    // with by the bit clear performed above--- it can only occur when we make
    // this `gpio_intr_enable()` call after the ISR has already triggered and
    // disabled itself, but before the other waiting task has recieved the
    // `IRQ_EVENT_GOT_IRQ` message and cleared the pending bit.
    gpio_intr_enable(src->pin_irq);

    // Now wait until the ISR tells us IRQ has gone active again.
    //
    // Note that it could have happened that after this `gpio_intr_enable()`
    // call, because of another wait operation in progress on another thread the
    // ISR had already triggered, disabled itself, and then sent the
    // `IRQ_EVENT_GOT_IRQ` bit. It is therefore imperative that the other
    // waiting task does not automatically clear the `IRQ_EVENT_GOT_IRQ` bit
    // upon its reciept. Else this could occur before we call
    // `xEventGroupWaitBits()` ourselves, thereby missing the setting of
    // `IRQ_EVENT_GOT_IRQ` which we wanted to observe. This would then lock up
    // this thread until another thread calls `sleep_until_irq_active()` again.
    //
    // As a solution, we simply don't ask FreeRTOS to automatically clear the
    // `IRQ_EVENT_GOT_IRQ` bit upon its reciept. Instead we perform the relevant
    // bit clear above, before the call to `gpio_intr_enable()`, which solves
    // all of our propblems.
    EventBits_t bits;
    do {
        bits =
            xEventGroupWaitBits(src->events,
...
static void irq_task(void* arg) {
    ...
    gpio_config_t io_conf = {
        .intr_type =
            src->trigger_high ? GPIO_INTR_HIGH_LEVEL : GPIO_INTR_LOW_LEVEL,
    ...
    EventBits_t bits;
    do {
        bits = xEventGroupWaitBits(src->events, IRQ_EVENT_SHOULD_TEARDOWN,
                                   false, false, portMAX_DELAY);
...
esp_err_t libirq_source_create(gpio_num_t pin_irq, bool trigger_high,
    ...
    char name_buff[32];
    snprintf(name_buff, sizeof(name_buff), "libirq(%u-%s)", pin_irq,
             trigger_high ? "HIGH" : "LOW");

    // Note that this task will allocate an interrupt, and therefore must happen
    // on a well-defined core (hence `xTaskCreatePinnedToCore()`), since
    // interrupts must be deallocated on the same core they were allocated on.
    //
    // After allocating the interrupt the task will put itself to sleep, only to
    // awake when it is time for the interrupt to be deallocated.
    BaseType_t result =
        xTaskCreatePinnedToCore(&irq_task, name_buff, IRQ_TASK_STACK_SIZE,
                                (void*) src, IRQ_TASK_PRIORITY, NULL, core_id);
    if (result != pdPASS) {
        free_irq_source(src);
...
void libirq_source_destroy(libirq_source_handle_t handle) {
    ...
    EventBits_t bits;
    do {
        bits = xEventGroupWaitBits(handle->events, IRQ_EVENT_SHOULD_TEARDOWN,
                                   false, false, portMAX_DELAY);
...
libirq_waiter_t libirq_waiter_create(libirq_source_handle_t handle) {
    // It is a race condition to create waiters after the source has been
    // destroyed. Also, this is pointless.
...
esp_err_t libirq_managed_loop_spawn(libirq_waiter_t waiter,
    ...
    esp_err_t ret =
        libtask_loop_spawn((libtask_do_once_fn_t) libirq_do_once_wrapper, data,
                           name, task_stack_size, task_priority, out_handle);
